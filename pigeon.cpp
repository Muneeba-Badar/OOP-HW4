...
void Pigeon::draw(){
    ...
    p1.moverRect = (SDL_Rect){x, y, 50, 50}; //moverRect remains the same
        p1.srcRect = (SDL_Rect){7,88,155,103}; //srcRect changed to pigeon up position
        pigeon.push_back(p1); //pushes the Unit type object into the vector of pigeon
}
...
Pigeon::Pigeon(){
    ...
    p1.srcRect = (SDL_Rect){7,88,155,103}; //up
    p1.srcRect = (SDL_Rect){0,237,153,84}; //middle
    p1.srcRect = (SDL_Rect){2,361,159,124}; //down


    // it will display pigeon on x = 30, y = 40 location, the size of pigeon is 50 width, 60 height
    p1.moverRect = (SDL_Rect){30, 40, 50, 50};
}
